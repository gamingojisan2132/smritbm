...
class BallGame extends Phaser.Scene {
        ...
        for (let i = 0; i < this.highScores.length; i++) {
            ...
            const dateText = this.add.text(
                ...
                scoreItem.date,
                {
                    fontFamily: 'Arial',
                    fontSize: '16px',
                    fill: isCurrentScore ? '#ffff00' : '#cccccc'
                }
            );
            dateText.setOrigin(0, 0.5);
            dateText.setDepth(102);
        ...
        if (!this.isFeverTime) {
            this.balls.getChildren().forEach(ball => {
                ...
                if (ballBottom >= slashTop && ballTop <= slashBottom) {
                    ...
                    this.scoreText.setText(`Score: ${this.score}`); // スコア表示更新
    ...
    startNewWave() {
        ...
        // 爆弾の出現（ウェーブ3-5で一回だけ）
        if (this.waveNumber >= 3 && this.waveNumber <= 5 && !this.bombAppeared) {
            // 50%の確率で爆弾を出現させる
    ...
    throwBomb() {
        ...
        // 爆弾が到達する頂点をランダムに決定（画面上部20%〜70%の間）
    ...
    triggerAllSlicedBonus() {
        ...
        const bonusText = this.add.text(
            this.sys.game.config.width / 2, 
            this.sys.game.config.height / 2 - 50, // 少し上に
            'お見事！', 
            {
                fontFamily: 'Arial Black', 
                fontSize: '48px', 
        ...
        bonusText.setOrigin(0.5);
        bonusText.setDepth(10); // 最前面に

        // ボーナススコア表示
        const scorePopup = this.add.text(
            this.sys.game.config.width / 2, 
            this.sys.game.config.height / 2 + 10, // 「お見事」の下に
            `スコア×110%`, // 表示をわかりやすく変更
            {
                fontFamily: 'Arial Black', 
                fontSize: '36px', 
        ...
        scorePopup.setOrigin(0.5);
        scorePopup.setDepth(10);
        
        // アニメーションで表示して消す
        this.tweens.add({
            targets: [bonusText, scorePopup],
            scale: 1.5,
            alpha: 0,
            duration: 2000,
            ease: 'Power1',
    ...
    endFeverMode() {
        ...
        this.cameras.main.fadeOut(1000, 255, 255, 255, (camera, progress) => {
            if (progress === 1) {
                // 5. フェードアウト完了後に次の通常ウェーブを開始
                this.cameras.main.resetFX(); // フェードアウト効果をリセット
                this.cameras.main.fadeIn(500, 255, 255, 255); // 次のウェーブ開始時にフェードイン
                this.waveStartScheduled = true; // スケジュール済みフラグを立てる
                this.time.delayedCall(100, this.startNewWave, [], this); // 短い遅延で次のウェーブへ
            }
        });
