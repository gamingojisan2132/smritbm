...
class BallGame extends Phaser.Scene {
    constructor() {
        ...
        // 同色ボーナス倍率の設定
        this.SAME_COLOR_BONUS_MULTIPLIER_2 = 1.5; // 同色2個の倍率
        this.SAME_COLOR_BONUS_MULTIPLIER_3 = 2.0; // 同色3個以上の倍率
    ...
    preload() {
        ...
        // リザルト画面用の背景UI画像
        this.load.image('background_ui', 'assets/images/background_ui.png');
    ...
    create() {
        ...
        this.ballColorSettings.forEach((setting, index) => {
            ...
            // カウント数字を表示（ボールの中央に配置）
            const text = this.add.text(x, startY, '0', { 
        ...
        // れんげき表示用のコンテナを作成（固定位置）
        this.comboTextContainer = this.add.container(width / 2, height * 0.25); // タイムカウントより下に移動
        this.comboTextContainer.setDepth(100);
        
        // 同色ボーナス表示用のコンテナを作成（固定位置）
        this.colorBonusContainer = this.add.container(width / 2, height * 0.30); // れんげき表示の下に移動
        this.colorBonusContainer.setDepth(100);
    ...
    // 結果画面の表示
    showResult() {
        // 結果画面のコンテナを作成（アニメーション用）
        const resultContainer = this.add.container(this.sys.game.config.width / 2, this.sys.game.config.height / 2);
        resultContainer.setDepth(100);
        resultContainer.setScale(0.1); // 初期サイズを小さく
        resultContainer.setAlpha(0); // 初期状態は透明
        ...
        // 画面横幅いっぱいにスケーリング
        panel.setDisplaySize(this.sys.game.config.width, this.sys.game.config.width * (panel.height / panel.width));
        resultContainer.add(panel);
        
        // ハイスコアリスト
        // 今回のスコアの10%下に配置
        const currentScoreY = -this.sys.game.config.height * 0.1;
        let yPos = currentScoreY + (this.sys.game.config.height * 0.1); // 今回スコアの10%下に配置
        
        // トップ5スコアのフォントサイズを定義
        const topScoreFontSize = 30; // 1.5倍に増加 (20 → 30)
        
        // スコアリストの最大幅を計算するための変数
        let maxScoreWidth = 0;
        let scoreItems = [];
        
        // まず全てのスコアアイテムを作成して最大幅を計算
        for (let i = 0; i < this.highScores.length; i++) {
            ...
            // スコアアイテムのコンテナを作成
            const scoreItemContainer = this.add.container(0, yPos);
            ...
            const rankText = this.add.text(
                ...
                {
                    fontFamily: 'Yusei Magic',
                    fontSize: `${topScoreFontSize}px`,
                    fill: isCurrentScore ? '#FFD700' : '#000000', // 黒、ハイスコア時は金色
                    stroke: '#ffffff',
                    strokeThickness: isCurrentScore ? 2 : 0
                }
            );
            rankText.setOrigin(0, 0.5);
            scoreItemContainer.add(rankText);
            
            const scoreItemText = this.add.text(
                rankText.width + 10, // ランク表示の右側に配置
                0,
                `${scoreItem.score}点`,
                {
                    fontFamily: 'Yusei Magic',
            ...
            scoreItemContainer.add(scoreItemText);
            
            // このスコアアイテムの合計幅を計算
            const totalWidth = rankText.width + 10 + scoreItemText.width;
            if (totalWidth > maxScoreWidth) {
                maxScoreWidth = totalWidth;
            }
            
            scoreItems.push({
                container: scoreItemContainer,
        ...
        for (let item of scoreItems) {
            // コンテナの位置を調整して中央揃えにする
            item.container.x = -item.totalWidth / 2;
            resultContainer.add(item.container);
        }
        
        // スコア表示（数字のみ）- トップ5の上に配置し、サイズを1.5倍に
        const scoreText = this.add.text(
            0,
            -this.sys.game.config.height * 0.1, // コンテナ内での相対位置
            `${this.score}`,
            {
                fontFamily: 'Yusei Magic',
                fontSize: `${Math.floor(topScoreFontSize * 1.5)}px`, // トップ5フォントの1.5倍
                fill: '#ffffff',
                stroke: '#000000',
                strokeThickness: 3
            }
        ...
        scoreText.setAlpha(0); // 初期状態は非表示
        resultContainer.add(scoreText);
        
        // リトライボタン（画像に変更）
        // スコアリストの最後の位置から10%下に配置
        const lastScoreYPos = yPos; // スコアリストの最後のY位置
        const retryButton = this.add.image(
            0,
            lastScoreYPos + (this.sys.game.config.height * 0.1), // スコアリストから10%下
            'retry'
        );
        // 画像サイズそのままに、画面幅に対してスケーリング
        const retryButtonScale = this.sys.game.config.width * 0.25 / retryButton.width;
        retryButton.setScale(retryButtonScale);
        retryButton.setInteractive({ useHandCursor: true });
        resultContainer.add(retryButton);
        ...
        retryButton.on('pointerover', () => {
            retryButton.setScale(retryButtonScale * 1.1);
        });
    ...
    displayCombo(comboCount, x, y) {
        // 既存のコンボテキストをクリア
        this.comboTextContainer.removeAll(true);
        
        // コンボテキストを作成
        const comboText = this.add.text(0, 0, `${comboCount}れんげき！`, {
            fontFamily: 'Yusei Magic',
            fontSize: '32px',
            fill: '#ffff00',
            stroke: '#000000',
        ...
        // コンテナに追加
        this.comboTextContainer.add(comboText);
    ...
    createSlash(pointer) {
        ...
        // このウェーブで既に斬撃済みの場合は何もしない (フィーバー中はスキップ)
        ...
        if (!this.isFeverTime) {
            this.balls.getChildren().forEach(ball => {
                ...
                if (ballBottom >= slashTop && ballTop <= slashBottom) {
                    ...
                    scorePopup.setDepth(100); // 最前面に表示
                    this.tweens.add({
                        targets: scorePopup,
        ...
        if (this.isFeverTime) {
            this.coins.getChildren().forEach(coin => {
                ...
                const coinDiameter = 32; // 仮。実際のサイズ変数を使うべき
        ...
        if (!this.isFeverTime && hitBall) { // ボールを斬った場合のみ連撃判定
            ...
            // 同色ボーナス倍率を計算
            let colorBonusMultiplier = 1.0; // デフォルトは倍率なし
            if (maxSameColorHit >= 3) {
                colorBonusMultiplier = this.SAME_COLOR_BONUS_MULTIPLIER_3; // 3個以上なら2倍
            } else if (maxSameColorHit >= 2) {
                colorBonusMultiplier = this.SAME_COLOR_BONUS_MULTIPLIER_2; // 2個なら1.5倍
            }
            
            // 同色ボーナスがある場合、スコアを追加
            if (colorBonusMultiplier > 1.0) {
                // 基本スコア10点 × 切ったボール数 × (倍率 - 1)
                // 例: 3個同色で切った場合、10 × 3 × (2.0 - 1.0) = 30点の追加ボーナス
                const bonusScore = Math.floor(10 * hitCount * (colorBonusMultiplier - 1.0));
                this.score += bonusScore;
                this.scoreText.setText(`${this.score}`); // スコア表示更新
                
                // 既存の同色ボーナステキストをクリア
                this.colorBonusContainer.removeAll(true);
                
                // 同色ボーナス表示
                const bonusText = this.add.text(0, 0, `同色ボーナス +${bonusScore}`, {
                    fontFamily: 'Yusei Magic',
                    fontSize: '24px',
                    fill: '#00ffff', // 水色で表示
                    stroke: '#000000',
                ...
                // コンテナに追加
                this.colorBonusContainer.add(bonusText);
            ...
            // 2個以上ボールを斬ったらコンボボーナス (色に関係なく)
            if (hitCount >= 2) {
                // コンボ表示 (3連撃以上でタイムボーナス)
                this.displayCombo(hitCount, comboX, comboY);
                if (hitCount >= 3) {
    ...
    throwBomb() {
        ...
        // 爆弾の当たり判定を半分に縮小（見た目はそのまま）
        bomb.body.setSize(bomb.width / 2, bomb.height / 2);
        bomb.body.setOffset(bomb.width / 4, bomb.height / 4);
        
        // デバッグ表示：当たり判定を赤枠で表示
        const debugGraphics = this.add.graphics();
        debugGraphics.lineStyle(2, 0xff0000);
        debugGraphics.strokeRect(
            bomb.x - bomb.body.width / 2,
            bomb.y - bomb.body.height / 2,
            bomb.body.width,
            bomb.body.height
        );
        debugGraphics.setDepth(20); // 爆弾より前面に表示
        bomb.debugGraphics = debugGraphics; // 爆弾に参照を保存
        ...
        // 重力を設定
        bomb.body.setGravityY(this.GRAVITY);
        ...
        this.time.addEvent({
            delay: 16, // 約60FPS
            callback: () => {
                if (bomb.active && bomb.debugGraphics) {
                    bomb.debugGraphics.clear();
                    bomb.debugGraphics.lineStyle(2, 0xff0000);
                    bomb.debugGraphics.strokeRect(
                        bomb.x - bomb.body.width / 2,
                        bomb.y - bomb.body.height / 2,
                        bomb.body.width,
                        bomb.body.height
        ...
        // 爆弾の初速度を計算
        const baseVelocityY = -Math.sqrt(2 * this.GRAVITY * (y - (this.sys.game.config.height * 0.5)));
    ...
    startFeverMode() {
        ...
        const feverText = this.add.text(
            ...
            'FEVER!!!',
            {
                fontFamily: 'Yusei Magic', // 目立つフォント
                fontSize: '100px',
                fill: '#ff0000', // 赤
                stroke: '#ffff00',
                strokeThickness: 8
    ...
    throwBackgroundImage() {
        ...
        bgImage.setDisplaySize(gameWidth, gameWidth * (bgImage.height / bgImage.width));
